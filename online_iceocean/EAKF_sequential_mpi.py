...
selected_variables = range(2) #just do both hemispheres in parallel                                                                                                                                                   
if COMM.rank == 0:
...
results_data = []
results_ID = []
for h,hem in scattered_jobs:
    prior_temp = np.copy(prior)
    if os.path.exists(hemispheres[h]):
    ...
    diff = np.where(prior!=prior_temp)
    results_data.append(prior[diff]-prior_temp[diff])
    results_ID.append(diff)
results_data = COMM.gather(results_data, root=0)
results_ID = COMM.gather(results_ID, root=0)

if COMM.rank == 0:
    results_data = list(itertools.zip_longest(*results_data))
    results_ID = list(itertools.zip_longest(*results_ID))
    increments = np.zeros((nmembers,nCat,xT*yT))

    for ix in range(len(results_data)):
        for jx in range(len(results_data[ix])):
            if results_data[ix][jx] is not None:
                increments[results_ID[ix][jx]] = results_data[ix][jx]

    increments = increments.reshape(nmembers,1,nCat,xT,yT)
