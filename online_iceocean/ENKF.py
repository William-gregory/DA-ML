...
def haversine(coords,localisation_radius=0.03):
    """
...
def Kfilter(prior,obs,lon,lat,loc_rad,obs_error):
    ...
    K = W * (Bm @ np.linalg.inv(Bo)) #localised Kalman gain
    posterior = np.array([prior[x] + (K @ (obs - priorH[x])) for x in range(E)])

    return posterior.reshape(E,C,dX,dY), (posterior-prior).reshape(E,C,dX,dY)
...
ocn_restart = sorted(glob.glob('MOM.res.*'))
nmembers = len(ice_restarts)
...
obs = xr.open_dataset('/gpfs/f5/gfdl_o/proj-shared/obs/NTSIC/???').sic.to_numpy()

selected_variables = range(len(grid)) #the number of tasks to be done
if COMM.rank == 0:
...
results = []
for index in scattered_jobs: #each computer node will execute this loop and send its segement of data to the ENKF
    outputs = Kfilter(prior[:,:,index:index+32,index+index+36],obs[index:index+32,index+index+36],\
                      lon[index:index+32,index+index+36],lat[index:index+32,index+index+36],loc_rad=localization_radius,obs_error=observation_error)
...
if COMM.rank == 0: #tell the master node to compile the results into their own respective arrays and map back to the 2D domain
    ...
    for r1 in results:
        for r2 in r1:
            if r2:
                posterior.append(r2[0])
                increments.append(r2[1])
...
ds = xr.Dataset(data_vars=dict(dSICN=(['member','time', 'ct', 'yT', 'xT'], dSICN)), coords=dict(yT=f['yT'], xT=f['xT']))
